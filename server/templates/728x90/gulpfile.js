...
function createZipArchive() {
  return new Promise((resolve, reject) => {
    const output = fs.createWriteStream('./728x90.zip');
...
async function enforceSizeLimit() {
  ...
  if (zipBytes > TARGET_ZIP_SIZE_KB * 1024) {
    console.warn(`[WARNING] Failed to reduce 728x90.zip under ${TARGET_ZIP_SIZE_KB}kb even at minimum quality floor! Result: ${Math.round(zipBytes/1024)}kb.`);
  } else {
    console.log(`[SUCCESS] Output 728x90.zip size: ${Math.round(zipBytes/1024)}kb`);
  }
}
