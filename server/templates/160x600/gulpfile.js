...
function createZipArchive() {
  return new Promise((resolve, reject) => {
    const output = fs.createWriteStream('./160x600.zip');
...
async function enforceSizeLimit() {
  ...
  if (zipBytes > TARGET_ZIP_SIZE_KB * 1024) {
    console.warn(`[WARNING] Failed to reduce 160x600.zip under ${TARGET_ZIP_SIZE_KB}kb even at minimum quality floor! Result: ${Math.round(zipBytes / 1024)}kb.`);
  } else {
    console.log(`[SUCCESS] Output 160x600.zip size: ${Math.round(zipBytes / 1024)}kb`);
  }
}
